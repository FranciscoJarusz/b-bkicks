        ...
        canvas.toBlob(
            (blob) => {
                if (!blob) {
                    reject(
                        new Error(
                            "No pudimos convertir la imagen antes de subirla."
                        )
                    );
                    return;
                }
        ...
        const { data: authListener } = supabase.auth.onAuthStateChange(
            (_event, nextSession) => {
                ...
                if (!nextSession) {
                    setProductos([]);
                    setStatus(
                        "Ingresa con tu cuenta para administrar el stock."
                    );
                }
            }
        );
    ...
    async function loadProductos() {
      ...
      `
        ...
        const normalized = (data ?? []).map((producto) => ({
            ...producto,
            producto_talle: [...(producto.producto_talle ?? [])].sort(
                sortSizes
            ),
        }));
                ...
                <div className="mx-auto grid w-full max-w-6xl items-center gap-20 lg:gap-60 lg:grid-cols-[1.1fr_0.9fr]">
                    <div className="flex flex-col justify-center">
                        ...
                        <h1 className="mt-4 max-w-xl font-accent text-5xl uppercase leading-none text-primary">
                            Controla stock, talles y productos desde un solo
                            lugar.
                        </h1>
                        <p className="mt-6 max-w-xl text-sm leading-6 text-black">
                            Este panel trabaja directo contra Supabase. Para que
                            quede realmente seguro, necesitas policies RLS y
                            permisos del bucket para escribir solo al admin
                            autenticado.
                        </p>
                    </div>
        ...
        <section className="min-h-screen px-4 py-6 text-black md:px-6">
            <div className="flex flex-col gap-20 mx-auto max-w-7xl">
                <div className="rounded-4xl py-10">
                    ...
                    {status && (
                        <p className="mt-4 text-sm text-black/50">{status}</p>
                    )}
                ...
                <div className="mt-6 overflow-hidden ring-1 ring-black/10">
                    ...
                    {productosVisibles.map((producto) => {
                        ...
                        const newVariant =
                            newVariantForms[producto.id_producto] ??
                            initialVariantForm;
                        const imageInputKey =
                            productImageKeys[producto.id_producto] ?? 0;
                        const queuedFiles =
                            productImageFiles[producto.id_producto] ?? [];
                        const totalStock = (
                            producto.producto_talle ?? []
                        ).reduce(
                            (sum, item) => sum + Number(item.stock ?? 0),
                            0
                        );
                        const isExpanded =
                            expandedProductId === producto.id_producto;
                                ...
                                <div className="grid gap-3 px-4 py-4 md:grid-cols-[32px_minmax(0,1.6fr)_130px_120px_120px_140px] md:items-center">
                                    ...
                                    <div className="text-sm font-semibold text-black">
                                        ...
                                        <p className="text-xs text-black/50">
                                            {producto.producto_talle.length}{" "}
                                            talle/s
                                        </p>
                                    </div>
                                    ...
                                    <div className="flex gap-2 md:justify-end">
                                        ...
                                        <button
                                            ...
                                            disabled={
                                                pendingKey ===
                                                `delete-product-${producto.id_producto}`
                                            }
                                            onClick={() =>
