...
function normalizeProducts(rows) {
    ...
    for (const row of rows) {
        ...
        const images = [
            ...new Set(
                (row.images ?? (row.image ? [row.image] : [])).filter(Boolean)
            ),
        ];
        ...
        for (const talle of talles) {
            const current = existing.talles.find(
                (item) => item.nombre === talle.nombre
            );
            if (current) {
...
async function main() {
    ...
    for (const product of products) {
        ...
        summary.imagesInserted += await syncProductImages(
            supabase,
            productId,
            product.images
        );
    ...
    console.log(
        "Importacion lista. producto_imagen guarda todas las imagenes y producto.imagen_url conserva la principal."
    );
}
